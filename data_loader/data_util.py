...
class EdgeBatchIterator(object):
    ...
    def batch_feed_dict(self, batch_idx):
        ...
        def extract_node_id(arr):
            if isinstance(arr, tuple):
                return arr[0]
            elif isinstance(arr, list) or isinstance(arr, np.ndarray):
                return [extract_node_id(i) for i in arr]
            else:
                return arr
        batch_from = extract_node_id(batch_from)
        batch_to = extract_node_id(batch_to)

        batch_context_from = extract_node_id(batch_context_from)
        batch_context_to = extract_node_id(batch_context_to)
    ...
    def sample(self, node_id, timestamp):
        ...
        for k in range(len(self.layer_infos)):
            ...
            for itid, ts in samples[k]:
                ...
                # support_sizes[k] * num_samples
                # choice_idx = np.random.choice(
                #     len(adj_list), size=num_samples, replace=replace)
                # nodes.extend(np.array(adj_list, dtype=tuple)[choice_idx])
            # [layer len, support_sizes[i], num_samples]
...
def aggregate(input_features, dims, placeholders,
              support_sizes, batch_size, aggregator):
    hidden = []
    for k in range(len(support_sizes)):
        layer_i = []
        for i in range(support_sizes[i]):
            tf.reshape(placeholders['batch_from'])
            aggregator()
            placeholders['batch_from_%d' % k][i]
        layer[i].append()
    hidden = [tf.embedding_lookup(input_features, ) for]
    return None
...
if __name__ == '__main__':
    ...
    input_features = tf.Variable(tf.constant(
        nodes, dtype=tf.float32), trainable=False)
    dims = [nodes.shape[1], 128, 128]
    output = aggregate(input_features, dims, placeholders,
                       support_sizes, FLAGS.batch_size, MeanPoolingAggregator)
    t = 0
